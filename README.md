...
配置数据库连接参数：复制 [`.env.example`](./.env.example) 为 `.env` 并填写真实值。
...
本项目在业务数据层之上集成了 NL2SQL 问数能力，架构参照「掌柜问数」：元数据知识库（MySQL + Qdrant + Elasticsearch）+ LangGraph 智能体 + DeepSeek 大模型。

### 架构说明

1. **元数据库 `meta`**：存储表/字段/指标语义信息（`table_info`、`column_info`、`metric_info`、`column_metric`）
2. **向量索引 Qdrant**：对字段和指标做语义召回（collection: `finance-agent-column`、`finance-agent-metric`）
3. **全文索引 Elasticsearch**：对维度字段取值做关键词匹配（index: `finance_agent_values`）
4. **问数智能体**：关键词提取 → 三路召回 → 过滤 → SQL 生成/校验/修正 → 执行
5. **查询 API**：`POST /api/query`（SSE 流式返回执行进度与结果）

### 环境依赖

除 MySQL 外，还需部署并开放以下服务。敏感配置统一放在 [`.env`](./.env)（可参考 [`.env.example`](./.env.example)），**不要提交到 Git**：

- Qdrant（默认 6333，`QDRANT_HOST` / `QDRANT_PORT`）
- Elasticsearch（默认 9200，需安装 IK 分词器，`ES_HOST` / `ES_PORT`）
- Text Embedding Inference（默认 8081，`TEI_HOST` / `TEI_PORT`）
- DeepSeek API Key（`DEEPSEEK_API_KEY`）

### 问数系统启动步骤

```bash
cp .env.example .env   # 复制后填写真实配置
uv sync

# 1. 若 finance 业务库已有数据，仅初始化 meta 元数据库：
uv run init_meta.py

# 2. 构建元数据知识库（写入 meta + Qdrant + ES）：
...
**调试页面**：`http://127.0.0.1:8000/static/query.html`
...
| 类别 | 示例问题 |
|------|----------|
| 客户 | 本月新增客户数是多少、当前正常账户数是多少 |
| 交易 | 最近30天交易金额是多少、本月交易笔数是多少 |
| 理财 | 当前理财持仓规模是多少 |
| 信贷 | 本月贷款申请数是多少、本月放款金额是多少、当前贷款余额是多少 |
| 还款 | 本月还款金额是多少、当前逾期余额是多少 |
| 风控 | 最近7天风险事件有多少 |
| 催收 | 催收回收率是多少 |
| 维度 | 哪个分行开户数量最多、哪个渠道交易金额最高 |
| 趋势 | 不同风险等级客户分布如何、本月和上月交易金额对比 |

测试脚本：`tests/test_nl2sql_queries.py`（包含 16 条 golden questions）
